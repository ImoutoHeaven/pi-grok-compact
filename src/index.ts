...
import { capturePayload, isGrok, requestCompaction, resolveRoute, routeIdentity } from "./remote.ts";
    ...
    pi.on("session_before_compact", async (event, ctx) => {
      ...
      try {
        ...
        const result = await requestCompaction({
          route: resolveRoute(model, auth), model: model.id,
        ...
        if (!owned()) return { cancel: true };
        const details = createCheckpoint(routeId, result.output, keptMessages(event));
        return { compaction: {
        ...
        // Provider errors can echo request bodies or credentials. Keep UI errors generic.
        if (!event.signal.aborted) ctx.ui.notify("Grok server compaction failed; original history retained. Check the endpoint, credentials and context limit, then retry /compact.", "warning");
        return { cancel: true };
      } finally {
